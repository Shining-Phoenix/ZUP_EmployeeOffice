...
Процедура СоздатьКадровуюИсториюНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		Результат = ЛК_ОбменДаннымиСервер.СоздатьЗаписьКадровойИстории("", НастройкиСервиса, Токен, Выборка.Сотрудник, Выборка.Подразделение, Выборка.Должность, Выборка.ДатаНачала);
