...
Процедура ЗарегистрироватьЗаписьСсылочногоОбъекта(Объект) Экспорт
	...
	Если Не ЗначениеЗаполнено(ЛК_РаботаСФункциями.Результат(ЛК_ОбменДаннымиПовтИсп.НастройкиСервиса()).АдресСервера) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ЗарегистрироватьЗаписьРегистраСведений(НаборЗаписей) Экспорт  
	...
	Объект1СJSON = ЛК_РаботаСJSON.ЗаписьJSON(Объект1С);
