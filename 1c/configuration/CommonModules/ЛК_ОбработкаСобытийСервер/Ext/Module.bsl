...
Процедура ЗарегистрироватьЗаписьРегистраНакоплений(НаборЗаписей) Экспорт  
	...
	Если ИмяМетаданных = "НачисленияУдержанияПоСотрудникам"  Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					   ...
					   |	НачисленияУдержанияПоСотрудникам.Организация.НаименованиеСокращенное КАК ОрганизацияНаименование
		               |ИЗ
		...
		ТЗ.Свернуть("Сотрудник, ФизическоеЛицо, Организация, ОрганизацияНаименование, Период");
		...
		Для Каждого СтрокаТЗ Из Тз  Цикл
			...
			Структура.Вставить("Сотрудник",      				   СтрокаТЗ.Сотрудник);
			Структура.Вставить("Месяц",                            СтрокаТЗ.Период); 
			Структура.Вставить("ОрганизацияНаименование",          СтрокаТЗ.ОрганизацияНаименование);	
