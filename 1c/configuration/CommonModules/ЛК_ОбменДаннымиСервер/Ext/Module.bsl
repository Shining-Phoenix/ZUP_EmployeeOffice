...
Функция ВыполнитьОбменПроизвольнымиДанными(НастройкиСервиса, Токен)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.ТипОбъкта = ЛК_ОбменДаннымиПовтИсп.ТипыОбъектов().РасчетныйЛист Тогда
			...
			Результат = ЛК_ОбменДаннымиСервер.ДобавитьОбновитьРасчетныйЛист(НастройкиСервиса, Токен, ИД, Месяц, ОрганизацияСсылка, ФизическоеЛицоСсылка); 
...
Функция ДобавитьОбновитьРасчетныйЛист(НастройкиСервиса, Токен, ИД, МесяцРасчетногоЛистка, Организация, ФизическоеЛицо) Экспорт 
	...
	ДанныеРасчетногоЛистка = Отчеты.АнализНачисленийИУдержаний.ДанныеРасчетныхЛистков(МассивФизическихЛиц, Организация, НачалоПериода, ОкончаниеПериода, Неопределено);
	...
	Если ДанныеРасчетногоЛистка.ДанныеРасчетныхЛистков <> Неопределено Тогда
		...
		ТЗ.Свернуть("Сотрудник, ВидРасчета, Группа, ПериодДействияПредставление," 
			+ "ПредставлениеРегистратораВыплаты, ОплаченныеДниЧасы," 
			+ "ОтработаноДней, ОтработаноЧасов", "Сумма");
		...
		Для Каждого СтрокаТЗ Из ТЗ Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЗ.ПредставлениеРегистратораВыплаты) Тогда 
				Структура.Вставить("payment_position", СтрокаТЗ.ПредставлениеРегистратораВыплаты);	
			Иначе 
			...
			Если ТипЗнч(СтрокаТЗ.ПериодДействияПредставление) = Тип("Дата") Тогда
				Структура.Вставить("validity",     Формат(СтрокаТЗ.ПериодДействияПредставление, "ДФ='МММ гггг'"));
	       	Иначе		
				Структура.Вставить("validity",     СтрокаТЗ.ПериодДействияПредставление);
			КонецЕсли;	
			
			Структура.Вставить("prioritet",  	   Строка(Сч)); 
			Структура.Вставить("hours_or_days",    СтрокаТЗ.ОплаченныеДниЧасы); 
			Структура.Вставить("days", 		       СтрокаТЗ.ОтработаноДней);
			Структура.Вставить("hours",            СтрокаТЗ.ОтработаноЧасов);
